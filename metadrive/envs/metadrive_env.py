...
METADRIVE_DEFAULT_CONFIG = dict(
    ...
    # ===== PG Map Config =====
    map=3,  # block数量，int or string: an easy way to fill map_config
    ...
    # ===== Others =====
    use_AI_protector=False,  # 仅在manual_control=True时生效
    save_level=0.5,  # AI介入力度
    horizon=1000,

    # ===== Agent =====
    random_spawn_lane_index=True,  # 随机选择出生车道
    vehicle_config=dict(navigation_module=NodeNetworkNavigation),
    agent_configs={
        DEFAULT_AGENT: dict(
            use_special_color=True,
            spawn_lane_index=(FirstPGBlock.NODE_1, FirstPGBlock.NODE_2, 0),
        )  # 默认出生在第一个 block 的第0车道（可能会被随机覆盖）
    },
