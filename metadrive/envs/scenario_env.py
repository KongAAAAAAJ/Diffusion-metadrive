"""
* 专门用于加载和回放真实世界驾驶数据集（如 nuScenes、Waymo）中的场景
...
SCENARIO_ENV_CONFIG = dict(
    ...
    num_scenarios=3,  # *场景池范围
    sequential_seed=False,  # *False=随机采样；True=顺序推进（多Worker必须True）
    ...
    reactive_traffic=False,  # *False=轨迹回放；True=IDM主动行驶
    ...
    crash_vehicle_done=False,   # *碰车默认不终止
