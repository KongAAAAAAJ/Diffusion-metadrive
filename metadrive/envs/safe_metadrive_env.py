...
class SafeMetaDriveEnv(MetaDriveEnv):
    def default_config(self) -> Config:
        ...
        config.update(
            {
                "num_scenarios": 100,
                "accident_prob": 0.8,  # *高概率出现障碍物，增加危险场景
                "traffic_density": 0.05,
                "crash_vehicle_done": False,  # *碰车不终止，允许带代价继续
                "crash_object_done": False,
                "cost_to_reward": False,  # *代价不折算进奖励（独立信号），在Safe RL中，奖励和代价分离，策略在满足代价约束（如每回合 cost ≤ 25）的前提下最大化奖励，两者同时优化但目标不同
                "horizon": 1000,
            },
