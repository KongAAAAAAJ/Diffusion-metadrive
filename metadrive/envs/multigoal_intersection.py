"""
*离线多目标评估：虽然 RL 训练时每回合只优化一个目标，但 info 中保留了所有目标的完整数据，可在不重新采集的情况下评估策略对4个目标的表现。
*目标条件策略：通过改变 obs（包含目标相关的导航信息），策略天然接收到目标信息，可训练出**目标条件（goal-conditioned）**策略。
*概率可调：修改 goal_probabilities 即可控制训练时各目标的采样频率，例如让困难目标（如 U 形掉头）出现更多次。
