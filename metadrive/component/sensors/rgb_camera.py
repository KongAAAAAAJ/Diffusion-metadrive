...
class RGBCamera(BaseCamera):
    ...
    def _setup_effect(self):
        ...
        multisample_candidates = [4] if is_mac() else [16]
        if 0 not in multisample_candidates:
            multisample_candidates.append(0)
        for multisamples in multisample_candidates:
            fbprops.set_multisamples(multisamples)
            tonemap_quad = self.manager.render_scene_into(colortex=self.scene_tex, fbprops=fbprops)
            if tonemap_quad is not None:
                self.tonemap_quad = tonemap_quad
                break

        if self.tonemap_quad is None:
            raise RuntimeError("RGBCamera failed to create tonemap quad for the current offscreen buffer")
        #
        defines = {}
        if is_mac():
