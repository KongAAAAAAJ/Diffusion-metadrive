...
class FreeRamp(Ramp):
    ...
    def _add_branch_socket_from_road(self, road: Road):
        CreateAdverseRoad(
            road,
            self.block_network,
        ...
        branch_socket = self.create_socket_from_positive_road(road)
        branch_socket.is_one_way = True
        self.add_sockets(branch_socket)
...
class FreeInRampOnStraight(FreeRamp):
    ID = "g"
    EXTRA_PART = InRampOnStraight.EXTRA_PART
    SOCKET_LEN = InRampOnStraight.SOCKET_LEN
    ...
    def _try_plug_into_previous_block(self) -> bool:
        ...
        merge_lane = self._get_merge_part(acc_lane, extension_length)
        self.block_network.add_lane(next_node, acc_road.end_node, merge_lane)
        self._add_branch_socket_from_road(branch_road)

        return no_cross


class FreeOutRampOnStraight(FreeRamp):
    ID = "G"
    EXTRA_LEN = OutRampOnStraight.EXTRA_LEN
    ...
    def _try_plug_into_previous_block(self) -> bool:
        ...
        self._add_branch_socket_from_road(straight_road)
        return no_cross
