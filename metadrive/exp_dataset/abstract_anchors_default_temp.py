...
FLATTENED_Y_MODE_INDICES = (0, 1, 4, 7)
...
def flatten_selected_modes_y(anchors: np.ndarray, mode_indices: Sequence[int] = FLATTENED_Y_MODE_INDICES) -> np.ndarray:
	flattened = anchors.copy()
	for mode_idx in mode_indices:
		if 0 <= mode_idx < flattened.shape[0]:
			flattened[mode_idx, :, 1] = 0.0
	return flattened
...
def generate_plan_anchors(
	...
	anchors = flatten_selected_modes_y(anchors)
