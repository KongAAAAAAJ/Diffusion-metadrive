...
class TransfuserBackbone(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        # 从预训练权重加载图像编码器
        local_weight = Path(config.bkb_path) if config.bkb_path else None
        if local_weight and local_weight.exists() and config.image_architecture in str(local_weight):
        ...
        # 如果包含地面高度信息，每帧激光雷达有两组特征（如强度+高度），所以通道数是 2 × config.lidar_seq_len
        if config.use_ground_plane:
            in_channels = 2 * config.lidar_seq_len  # 通道数
        else:
        ...
        self.global_pool_lidar = nn.AdaptiveAvgPool2d(output_size=1)  # 把任意输入的激光雷达特征图池化为1×1的空间尺寸
