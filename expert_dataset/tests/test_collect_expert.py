...
def _load_module():
    ...
    expert_dataset_package = ModuleType("expert_dataset")
    expert_dataset_package.__path__ = []
    policy_package = ModuleType("metadrive.policy")
    policy_package.__path__ = []
    diffusion_policy_package = ModuleType("models.diffusion")
    diffusion_policy_package.__path__ = []
    sys.modules.setdefault("metadrive", metadrive_package)
    sys.modules.setdefault("expert_dataset", expert_dataset_package)
    sys.modules.setdefault("metadrive.policy", policy_package)
    sys.modules.setdefault("models.diffusion", diffusion_policy_package)
    ...
    expert_idm_module = ModuleType("expert_dataset.expert_idm_policy")
    ...
    env_module = ModuleType("envs.diffusion_envs.base_multi_env")
    ...
    trajectory_module = ModuleType("expert_dataset.trajectory_correction")
    ...
    transfuser_features_module = ModuleType("models.diffusion.transfuser_features")
    transfuser_features_module.BoundingBox2DIndex = type(
        "BoundingBox2DIndex",
        (),
        {"size": staticmethod(lambda: 5), "POINT": slice(0, 2)},
    )
    mode_context_module = ModuleType("models.diffusion.mode_context")
    ...
    mode_labeler_module = ModuleType("models.diffusion.mode_labeler")
    ...
    mode_trajectory_generator_module = ModuleType("models.diffusion.mode_trajectory_generator")
    mode_trajectory_generator_module.ModeTrajectoryGenerator = _StubModeTrajectoryGenerator

    mode_visualization_module = ModuleType("models.diffusion.mode_visualization")
    ...
    transfuser_config_module = ModuleType("models.diffusion.transfuser_config")
    ...
    dataset_module = ModuleType("expert_dataset.metadrive_dataset")
    dataset_module.split_shards = lambda **kwargs: {"train": [], "val": [], "test": []}

    trajectory_filter_module = ModuleType("expert_dataset.trajectory_filter")
    ...
    scenario_orchestrator_module = ModuleType("expert_dataset.scenario_orchestrator")
    ...
    route_definitions_path = repo_root / "routes" / "route_definitions.py"
    ...
    scenario_definitions_path = repo_root / "scenarios" / "definitions.py"
    ...
    local_traffic_spawner_path = repo_root / "traffic" / "local_traffic_spawner.py"
    local_traffic_spawner_spec = importlib.util.spec_from_file_location(
        "traffic.local_traffic_spawner",
    ...
    if sys.modules.get("models.diffusion") is diffusion_policy_package:
        sys.modules.pop("models.diffusion", None)
    if sys.modules.get("metadrive.policy") is policy_package:
        sys.modules.pop("metadrive.policy", None)
    if sys.modules.get("expert_dataset") is expert_dataset_package:
        sys.modules.pop("expert_dataset", None)
