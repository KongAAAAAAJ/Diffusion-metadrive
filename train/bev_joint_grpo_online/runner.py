...
def run_joint_grpo_training(training_config: JointGRPOTrainingConfig, *, run_dir: Path) -> dict[str, object]:
    ...
    safety = training_config.safety_post_training
    ...
    grpo_config = JointGRPOConfig(
        ...
        steering_rate_feasibility_weight=feasibility.steering_rate_weight,
        wheelbase_m=feasibility.wheelbase_m,
        trajectory_dt_s=feasibility.trajectory_dt_s,
        min_segment_length_m=feasibility.min_segment_length_m,
        steering_initial_limit_deg=feasibility.steering_initial_limit_deg,
        steering_final_limit_deg=feasibility.steering_final_limit_deg,
        steering_rate_initial_limit_deg_s=feasibility.steering_rate_initial_limit_deg_s,
        steering_rate_final_limit_deg_s=feasibility.steering_rate_final_limit_deg_s,
        feasibility_schedule_power=feasibility.schedule_power,
        risk_pact_distill_weight=risk_pact.distill_weight,
        risk_pact_use_background_actor=risk.use_background_actor,
        risk_pact_use_platoon_actor=risk.use_platoon_actor,
        risk_pact_use_road_boundary=risk.use_road_boundary,
        risk_pact_horizon_dt_s=risk.horizon_dt_s,
        risk_pact_ego_length_m=risk.ego_length_m,
        risk_pact_ego_width_m=risk.ego_width_m,
        risk_pact_background_longitudinal_clearance_m=risk.background_longitudinal_clearance_m,
        risk_pact_background_lateral_clearance_m=risk.background_lateral_clearance_m,
        risk_pact_actor_temperature_m=risk.actor_temperature_m,
        risk_pact_actor_softmax_beta=risk.actor_softmax_beta,
        risk_pact_actor_confidence_decay_enabled=risk.actor_confidence_decay_enabled,
        risk_pact_actor_full_confidence_horizon_s=risk.actor_full_confidence_horizon_s,
        risk_pact_actor_confidence_decay_rate_per_s=risk.actor_confidence_decay_rate_per_s,
        risk_pact_actor_min_confidence=risk.actor_min_confidence,
        risk_pact_platoon_vehicle_length_m=risk.platoon_vehicle_length_m,
        risk_pact_platoon_vehicle_width_m=risk.platoon_vehicle_width_m,
        risk_pact_platoon_longitudinal_clearance_m=risk.platoon_longitudinal_clearance_m,
        risk_pact_platoon_lateral_clearance_m=risk.platoon_lateral_clearance_m,
        risk_pact_component_softmax_beta=risk.component_softmax_beta,
        risk_pact_road_safety_margin_m=risk.road_safety_margin_m,
        risk_pact_road_temperature_m=risk.road_temperature_m,
        risk_pact_temporal_softmax_beta=risk.temporal_softmax_beta,
        risk_pact_risk_threshold=risk.risk_threshold,
        risk_pact_violation_temperature=risk.violation_temperature,
        risk_pact_safe_margin=risk.safe_margin,
        risk_pact_teacher_step_m=risk.teacher_step_m,
        risk_pact_gradient_eps=risk.gradient_eps,
        risk_pact_gradient_clip_norm=risk.gradient_clip_norm,
        risk_pact_curriculum_start_scale=curriculum.start_scale,
        risk_pact_curriculum_end_scale=curriculum.end_scale,
        ...
        while accepted_update_states < target_accepted_update_states and (not diagnostic_early_stop) and (stability_guard_diagnostic is None):
            ...
            try:
                while accepted_update_states < target_accepted_update_states and (not diagnostic_early_stop) and (stability_guard_diagnostic is None) and (bucket_accepted_update_counts[bucket_index] < bucket_target_counts[bucket_index]) and (current_visit_progress < config.rollout_groups_per_bucket_visit) and (episode_step < config.environment_steps_per_episode):
                    ...
                    if start_target_step is None or episode_step < start_target_step:
                        ...
                        sampling_health = _sampling_health_metrics(sampling_attempts=sampling_attempts, rejected_sampling_attempts=rejected_sampling_attempts, exhausted_states=exhausted_states, visited_live_states=baseline_execution_steps)
                        for metric_name, metric_value in sampling_health.items(): writer.add_scalar(metric_name, metric_value, baseline_execution_steps)
                        last_metrics.update(sampling_health)
            ...
            if accepted_update_states == accepted_at_episode_start and baseline_execution_steps == baseline_steps_at_episode_start:
                if retryable_start_rejection:
                    ...
                    if consecutive_empty_episodes >= config.max_consecutive_empty_episodes:
                        raise OnlineGRPOError(f'{config.max_consecutive_empty_episodes} consecutive episodes produced no feasible online state rollout for training bucket {bucket_index}: {scenario[0]}/{scenario[1]} seed={seed}')
            else:
                consecutive_empty_episodes = 0
