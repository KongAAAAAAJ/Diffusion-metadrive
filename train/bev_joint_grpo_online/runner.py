...
from models.bev_planner import DEFAULT_DDIM_PATH, GRPO_OPEN_REWARD_APPLICATION_CONTRACT, GRPO_OPEN_REWARD_APPLICATION_CONTRACT_SHA256, JointGRPOConfig, JointRewardConfig, KinematicTrajectoryOptimizer, KinematicTrajectoryOptimizerConfig, TrajectoryOptimizationError, joint_grpo_optimizer_contract, joint_grpo_optimizer_contract_sha256
from models.bev_planner.joint_reward import VEHICLE_MODE_REWARD_CONTRACT, VEHICLE_MODE_REWARD_CONTRACT_SHA256, VehicleModeCounterfactualReward, VehicleModeRewardConfig, vehicle_mode_reward_config_sha256
...
from .contracts import BEST_CHECKPOINT_METRIC, _application_contract_version, _checkpoint_file_sha256, _checkpoint_payload, _frozen_pretrain_reward_logging_metadata, _grpo_config_artifact_payload, _implementation_commit, _performance_summary, _reward_contract_version, _sampler_state, _validate_online_checkpoint_metadata, _validate_raw_reward_config, rollout_collection_contract
...
from .validation import _FixedValidationFrozenEntry, _fixed_raw_proxy_and_simulator_validation, _load_grpo_validation_state_bank, _resume_best_checkpoint_anchor, _validation_is_safety_eligible, _validation_reward_comparison_metrics
def run_joint_grpo_training(training_config: JointGRPOTrainingConfig, *, run_dir: Path) -> dict[str, object]:
    ...
    pretrain_validation, pretrain_simulator_errors = _fixed_raw_proxy_and_simulator_validation(trainer, device=torch_device, reward_config=joint_diagnostic_reward_config, scenarios=config.scenarios, seeds=HOLDOUT_SEEDS, validation_state_bank=validation_state_bank, frozen_cache=fixed_validation_cache)
    validation_calls += 1
    ...
    best_unconstrained_path = run_dir / 'checkpoints' / 'best_simulator_unconstrained.pt'
    ...
    simulator_diagnostic_errors: list[dict[str, object]] = [{'optimizer_step': 0, 'stage1_baseline': True, **dict(value)} for value in pretrain_simulator_errors]
    ...
    try:
        while accepted_update_states < target_accepted_update_states and (not diagnostic_early_stop) and (stability_guard_diagnostic is None):
            ...
            if accepted_update_states != last_validated_update_state and (accepted_update_states == target_accepted_update_states or (accepted_update_states > 0 and accepted_update_states % config.validation_interval_rollouts == 0)):
                validation, simulator_errors = _fixed_raw_proxy_and_simulator_validation(trainer, device=torch_device, reward_config=joint_diagnostic_reward_config, scenarios=config.scenarios, seeds=HOLDOUT_SEEDS, validation_state_bank=validation_state_bank, frozen_cache=fixed_validation_cache)
                validation_calls += 1
                ...
                for value in simulator_errors:
                    error_record = {'optimizer_step': int(trainer.optimizer_step), 'accepted_update_state': int(accepted_update_states), 'stage1_baseline': False, **dict(value)}
                    simulator_diagnostic_errors.append(error_record)
                    writer.add_text('validation/simulator_error', json.dumps(error_record, sort_keys=True), accepted_update_states)
                ...
                if selection_score is not None:
                    validation['validation/simulator_reward_gain_trailing3'] = selection_score[0]
                    validation['validation/selected_reward_gain_trailing3'] = selection_score[1]
                last_metrics.update(validation)
                for metric_name in ('validation/checkpoint_safety_eligible', 'validation/simulator_reward_gain_trailing3', 'validation/selected_reward_gain_trailing3'):
                ...
                if accepted_update_states >= 200 and len(validation_selection_history) >= 3:
                    ...
                    diagnostic_early_stop = all((value['simulator_reward_gain'] < 0.0 and value['selected_reward_gain'] < 0.0 and (value['s7_out_delta'] > 0.0) for value in trailing))
    finally:
        writer.close()
    if stability_guard_diagnostic is not None:
        guard_sampler_state = _sampler_state(accepted_update_states=accepted_update_states, sampling_attempts=sampling_attempts, rejected_sampling_attempts=rejected_sampling_attempts, stability_guard_rejections=stability_guard_rejections, exhausted_states=exhausted_states, baseline_execution_steps=baseline_execution_steps, zero_signal_epochs=zero_signal_epochs, warmup_environment_steps=warmup_environment_steps, bucket_target_counts=bucket_target_counts, bucket_accepted_update_counts=bucket_accepted_update_counts, bucket_sampling_attempt_counts=bucket_sampling_attempt_counts, bucket_rejected_sampling_attempt_counts=bucket_rejected_sampling_attempt_counts, bucket_stability_guard_rejection_counts=bucket_stability_guard_rejection_counts, bucket_exhausted_state_counts=bucket_exhausted_state_counts, bucket_baseline_execution_step_counts=bucket_baseline_execution_step_counts, bucket_zero_signal_epoch_counts=bucket_zero_signal_epoch_counts, bucket_optimizer_step_counts=bucket_optimizer_step_counts, bucket_episode_counts=bucket_episode_counts, next_bucket_index=next_bucket_index, current_visit_progress=current_visit_progress, rollout_start_generator_state=rollout_start_generator.get_state(), last_validated_update_state=last_validated_update_state, rollout_groups_per_bucket_visit=config.rollout_groups_per_bucket_visit, optimizer_step=trainer.optimizer_step, environment_steps=environment_steps, max_sampling_attempts=max_sampling_attempts)
    ...
    plot_paths = generate_grpo_plots(run_dir / 'tb', run_dir / 'plots', require_frozen_pretrain_reward=True, rollout_axis_label=ACCEPTED_ROLLOUT_AXIS_LABEL)
