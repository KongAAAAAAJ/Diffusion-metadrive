...
def load_planner(
    ...
    if model_version != "v2":
        raise Stage1TrainingError(
            "Stage 1 checkpoint model_version must be explicitly v2"
        )
    planner_config = stage1_planner_config_from_mapping(header.get("planner_config"))
    if planner_config.predecessor_condition != VARIANT_CONDITION[str(variant)]:
        raise Stage1TrainingError(
            "Stage 1 checkpoint planner_config predecessor condition mismatch"
        )
    planner = BEVOnlyDiffusionPlanner(planner_config)
...
def validate_closed_loop(
    ...
    *,
    ...
    del planner, device, artifact_root, model_id, topdown_screen_size, topdown_film_size
    return {
