...
class ModeSelectionSB3Env(gym.Env):
    ...
    def step(self, action):
        ...
        masks = np.asarray(step_export["lane_valid_mask"], dtype=bool)
    ...
    def action_masks(self) -> np.ndarray:
        ...
        if self._last_export is None:
            return np.ones((self.num_agents * self._num_modes,), dtype=bool)
        return np.asarray(self._last_export["lane_valid_mask"], dtype=bool).reshape(-1)
    ...
    def _refresh_mode_export(self) -> dict[str, np.ndarray]:
        ...
        self._num_modes = int(np.asarray(export["lane_valid_mask"]).shape[1])
        self.action_space = gym.spaces.MultiDiscrete([self._num_modes] * self.num_agents)
        self.observation_space = self._make_observation_space()
        obs = {
            ...
            "agent_mode_masks": np.asarray(export["lane_valid_mask"], dtype=bool),
