...
**代码修改前硬门槛**：凡修改源代码、测试、配置、脚本、构建文件、依赖、接口或架构，必须先完整读取并执行 [`.agents/skills/simple-code/SKILL.md`](.agents/skills/simple-code/SKILL.md)。首次编辑前须在 commentary 中简要记录成功标准、最小修改范围、已知事实与未知项、验证命令及长任务/线程边界；未完成该记录不得修改。
