...
from models.diffusion.modules.blocks import gen_sineembed_for_position
...
class PlatoonDiffusionPlanner(nn.Module):
    ...
    def export_mode_selection(self, batch: Mapping[str, Mapping[str, Tensor]]) -> Dict[str, object]:
        ...
        return {
            ...
            "lane_valid_mask": mode_valid_mask.numpy(),
    ...
    def export_for_grpo(
        ...
        return {
            ...
            "lane_valid_mask": mode_valid_mask,                                # [N, M] bool Tensor
    ...
    def extract_rl_context(self, batch: Mapping[str, Mapping[str, Tensor]]) -> Tuple[Dict[str, dict], list]:
        ...
        for agent_id in agent_ids:
            ...
            cfg = getattr(self.model, "_config", None)
            tgt_type = getattr(cfg, "target_guidance_type", None)
            if tgt_type in ("point", "external_point") and "target_point" in sample:
                tp_tensor = self._ensure_batch_dim(sample["target_point"]).to(device=device).float()
                reg_target_embed = self.model._encode_target_guidance({"target_point": tp_tensor})
            ...
            contexts[agent_id] = {
                ...
                "reg_target_embed": reg_target_embed,          # [1, target_point_dim] or None
            }
    ...
    def rollout_selected_refinement(self, context: dict, selected_traj_np, **kwargs) -> Dict[str, Tensor]:
        ...
        """
        from train.train_refine_grpo import rollout_selected_refinement
    ...
    def predict_denoised_traj(
        ...
        reg_target_embed = context.get("reg_target_embed")  # [1, target_point_dim] or None
