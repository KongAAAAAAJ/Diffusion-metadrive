...
class LossComputer(nn.Module):
    ...
    def forward(self, poses_reg, poses_cls, targets, plan_anchor, mode_valid_mask=None):
        ...
        mode_valid_mask: (bs, num_mode) bool, optional — invalid slots get inf distance so they are never chosen as mode_idx
        """
        bs, num_mode, ts, d = poses_reg.shape
        target_traj = targets["trajectory"]

        # Select mode_idx as the anchor closest to the GT trajectory (average L2 over timesteps).
        # mode_valid_mask is intentionally ignored: the generator may mark lane-change slots
        # as invalid even when the vehicle is actively changing lanes (timing/pose mismatch).
