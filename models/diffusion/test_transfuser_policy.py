...
from envs.diffusion_envs.base_multi_env import DatasetCollectEnv
...
from models.diffusion.run_dir_utils import create_numbered_run_dir
...
DEFAULT_REFINE_TRAIN_CONFIG_PATH = "configs/train/refine_grpo.yaml"
...
def _build_pdms_reward_params(config: dict, args) -> dict:
    ...
    merged.setdefault("target_speed_km_h", env_config.get("target_speed_km_h", getattr(args, "target_speed_km_h", 30.0)))
    merged.setdefault("desired_gap_m", getattr(args, "desired_gap_m", 10.0))
    return build_platoon_metric_params(merged)
    ...
    for agent_index, agent_id in enumerate(agent_ids):
        ...
        if is_leader:
            ...
            formation_lon_scores, formation_lat_scores = compute_pairwise_formation_reward(
                ...
                lon_decay_m=float(params.get("lon_decay_m", 5.0)),
                lat_decay_m=float(params.get("lat_decay_m", 0.5)),
                progress_s_max=float(params["progress_s_max"]),
            )
        else:
            formation_lon_scores = np.zeros(1, dtype=np.float32)
            formation_lat_scores = np.zeros(1, dtype=np.float32)
        ...
        if coarse is not None:
            ...
            if coarse_np.ndim == 3 and 0 <= mode_idx < coarse_np.shape[0] and coarse_np.shape[2] >= 2:
                anchor_traj = coarse_np[mode_idx, :, :2][None]

        reward = compute_pdms_reward_batch(
            traj[None],
            pose,
            prev_traj,
            prev_pose,
...
def _attach_platoon_dynamic_mode_features(
    ...
    *,
    ...
    from models.diffusion.mode_definitions import mode_slot_count
    num_slots = mode_slot_count(
        config.mode_keep_lane_count,
        config.mode_lane_change_left_count,
        config.mode_lane_change_right_count,
        ...
        for episode_idx in range(args.episodes):
            ...
            while not done:
                ...
                if hasattr(env, "_pending_step_mode_groups"):
                    from models.diffusion.mode_definitions import build_mode_slots as _bms
